...
class AssessmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            {
                $data = Siswa::where('teacher_id',$this->getUserLogin()->id)->join('tbl_class', 'tbl_siswa.class_id', '=', 'tbl_class.id')->get(['tbl_siswa.id','siswa_name','memorization_type','class_id','nis']);
            }
            else
            {
                $data = Siswa::all();
